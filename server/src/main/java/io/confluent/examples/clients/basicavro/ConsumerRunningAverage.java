...
public class ConsumerRunningAverage {

    private static final String TOPIC = "temperature-average";
    ...
    public static void main(final String[] args) throws IOException {
        ...
        try (final KafkaConsumer<Long, Double> consumer = new KafkaConsumer<>(props)) {
            consumer.subscribe(Collections.singletonList(TOPIC));

            while (true) {
                final ConsumerRecords<Long, Double> records = consumer.poll(Duration.ofMillis(100));
                for (final ConsumerRecord<Long, Double> record : records) {
                    final Long key = record.key();
                    final Double value = record.value();
