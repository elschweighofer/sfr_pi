...
public class ConsumerTemperature {
    ...
    public static void main(final String[] args) throws IOException {
        ...
        try (final KafkaConsumer<String, Temperature> consumer = new KafkaConsumer<>(props)) {
            consumer.subscribe(Collections.singletonList(TOPIC));

            while (true) {
                final ConsumerRecords<String, Temperature> records = consumer.poll(Duration.ofMillis(100));
                for (final ConsumerRecord<String, Temperature> record : records) {
